...
class _TestSnippet(snippet.Jinja2Snippet):
  ...
  def __init__(self, template_string, is_applicable=True):
    super().__init__(jinja2.Template(template_string))
    self._is_applicable = is_applicable

  def is_applicable(self, context_):
...
class RegistryTest(unittest.TestCase):
  ...
  def test_tags(self):
    self._registry.register('foo', _TestSnippet('bar'))
    self._registry.register('zzz', _TestSnippet('quux', False))
    self.assertSequenceEqual(
        sorted(('foo',)), sorted(self._registry.tags(self._context)))
