...
class Registry:
  ...
  def _plugins_for_snippet(
      self, tag: str) -> Generator[plugin.SnippetPlugin, None, None]:
    """Yields all snippet plugins for a tag, recursively.

    Args:
      tag: Tag of a snippet.
    """
    for scope in _self_and_parent_tags(tag):
      yield from plugin.recurse(self._plugins[scope], plugin.SnippetPlugin)

  def variants(self, context_: context.Context,
               tag: str) -> Generator[Optional[str], None, None]:
    """Yields all applicable variants of the given tag.

    This includes the default variant (None) if it's applicable.
    ...
    """
    variants = set((None,))
    for plugin_ in self._plugins_for_snippet(tag):
      variants.update(plugin_.variants)
    for variant in variants:
      if all((x.is_applicable(context_, tag, variant)
              for x in self._plugins_for_snippet(tag))):
        yield variant
  ...
  def tags(self, context_: context.Context) -> Generator[str, None, None]:
    ...
    for tag in self._snippets.keys():
      for variant in self.variants(context_, tag):
        if variant is None:
          yield tag
        else:
...
def _walk_config_dirs(config_dirs: Iterable[str], top_level: str, extension: str
...
def load_python_plugins(registry: Registry, config_dirs: Iterable[str]) -> None:
  ...
  for top_level, dir_relpath, filename, tag in _walk_config_dirs(
    ...
    registry.register_plugin(
        tag,
        plugin.PythonMetaPlugin(os.path.join(top_level, dir_relpath, filename)))
...
def load_yaml_plugins(registry: Registry, config_dirs: Iterable[str]) -> None:
  ...
  for top_level, dir_relpath, filename, tag in _walk_config_dirs(
    ...
    registry.register_plugin(
        tag, plugin.YamlPlugin(os.path.join(top_level, dir_relpath, filename)))
...
def load_jinja2_snippets(registry: Registry,
                         config_dirs: Iterable[str]) -> None:
