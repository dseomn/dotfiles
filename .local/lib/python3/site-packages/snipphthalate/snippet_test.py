# Copyright 2019 Google LLC
#
# Licensed under the Apache License, Version 2.0 (the "License");
# you may not use this file except in compliance with the License.
# You may obtain a copy of the License at
#
#     https://www.apache.org/licenses/LICENSE-2.0
#
# Unless required by applicable law or agreed to in writing, software
# distributed under the License is distributed on an "AS IS" BASIS,
# WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied.
# See the License for the specific language governing permissions and
# limitations under the License.
"""Tests for snipphthalate.snippet."""

import unittest
from unittest import mock

import jinja2

from snipphthalate import context
from snipphthalate import plugin
from snipphthalate import snippet


class _FakeSnippet(snippet.Snippet):

  def render(self, *args, **kwargs):
    super().render(*args, **kwargs)


class SnippetTest(unittest.TestCase):

  def test_render_not_implemented(self):
    with self.assertRaises(NotImplementedError):
      _FakeSnippet().render(mock.create_autospec(context.Context), (), None)


class _TestJinja2SnippetPlugin(plugin.Jinja2SnippetPlugin):

  def __init__(self, **kwargs):
    """Initializer.

    Args:
      **kwargs: Template context.
    """
    super().__init__()
    self._kwargs = kwargs

  def update_context(self, jinja2_context, snipphthalate_context, variant):
    jinja2_context.update(self._kwargs)


class Jinja2SnippetTest(unittest.TestCase):

  def setUp(self):
    super().setUp()
    self._context = mock.create_autospec(context.Context)

  def test_system_context(self):
    self._context.__str__.return_value = 'kumquat'
    self.assertEqual(
        'kumquat',
        snippet.Jinja2Snippet(jinja2.Template('{{context}}')).render(
            self._context, (), None))

  def test_render(self):
    for subtest, path, template, plugins, expected in (
        ('no_vars', None, 'kumquat', (), 'kumquat'),
        ('plugin_var', None, '{{foo}}', (_TestJinja2SnippetPlugin(foo='bar'),),
         'bar'),
        ('system_var', 'quux', '{{context.path()}}', (), 'quux'),
    ):
      with self.subTest(subtest):
        self._context.path.return_value = path
        actual = snippet.Jinja2Snippet(jinja2.Template(template)).render(
            self._context, plugins, None)
        self.assertEqual(expected, actual)

  def test_render_error(self):
    with self.assertRaises(snippet.RenderError):
      snippet.Jinja2Snippet(jinja2.Template('{{foo.bar}}')).render(
          self._context, (), None)


if __name__ == '__main__':
  unittest.main()
